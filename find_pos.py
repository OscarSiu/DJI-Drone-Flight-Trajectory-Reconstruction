...
filename = 'input/20220518-0458.csv' #flight log
...
# Data Extraction
clock = df['time(millisecond)'][index:,].values/1000  #timestamp


velN= np.vstack(df[' xSpeed(m/s)'][index:].values)
#if velN[-1] == 0:
#velN= np.vstack(df[' xSpeed(m/s)'][index:].values)
velE=np.vstack(df[' ySpeed(m/s)'][index:].values)
velD=np.vstack(df[' zSpeed(m/s)'][index:].values)

#else:
#    velE=np.vstack(df['IMU_ATTI(0):velE'][index:].values)
#    velD=np.vstack(df['IMU_ATTI(0):velD'][index:].values)

gps_lat = df['latitude'][index:].values
gps_long = df['longitude'][index:].values
height = abs(df['height_above_takeoff(meters)'][index:].values)
